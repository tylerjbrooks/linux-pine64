...
//	struct snd_interval ranges[2];
    ...
    printk("JDS - pcm512x_hw_rule_rate %d\n", frame_size);
	if (frame_size < 0)
		return frame_size;
				      ...
				      struct snd_soc_dai *dai)
{
    ...
    printk("JDS pcm512x_dai_startup_master\n");
				     ...
				     struct snd_soc_dai *dai)
{
    ...
    printk("JDS pcm512x_dai_startup_slave\n");
	if (IS_ERR(pcm512x->sclk)) {
			       ...
			       struct snd_soc_dai *dai)
{
    ...
    printk("JDS - pcm512x_dai_startup %x\n", pcm512x->fmt);
			     ...
			     struct snd_soc_dai *dai)
{
    ...
    ret = pcm512x_dai_startup(substream, dai);
    if (ret)
        return ret;
